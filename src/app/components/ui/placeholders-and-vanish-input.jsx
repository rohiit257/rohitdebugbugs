...
export function PlaceholdersAndVanishInput({
  ...
  const handleKeyDown = async (e) => {
    if (e.key === "Enter" && !animating) {
      ...
      vanishAndSubmit();
      // Send message to API on Enter key press
      try {
        const response = await axios.post('/api/send_message', { message: value });
        if (response.status === 201) { // Status 201 indicates successful creation
          console.log('Message stored successfully:', response.data);
        } else {
          console.error('Error storing message:', response.data.message);
